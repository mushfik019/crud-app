...
class _HomePageState extends State<HomePage> {
    ...
    if (response.statusCode == 200) {
      ...
      if (decodedResponse['status'] == 'success') {
        if (mounted) {
          ...
          showSnackBar('Product added successful!');
        }
      } else {
        if (mounted) {
          showSnackBar('Failed to add new product');
        }
      }
    }
  }
  ...
  void updateProduct(String id) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (decodedResponse['status'] == 'success') {
        if (mounted) {
          showSnackBar('Product update successfull!');
        }
        getProduct();
      }
    } else {
  ...
  void showUpdateDialogueBox(Product product) {
    _nameTEController.text = product.name;
    _codeTEController.text = product.code;
...
class Product {
  final String id, name, code, quantity, unitPrice, totalPrice, image;
  Product(this.id, this.name, this.code, this.quantity, this.unitPrice,
      this.totalPrice, this.image);
